...
- 어노테이션 목록
  ...
  - @ResposeBody : 응답페이지에 텍스트 출력 어노테이션, 자바의 String 문자열 자동 생성된 웹페이지에 렌더링 작업
...
- 방법
  - jsp 페이지 : 가장 구식 방식. 예전 Spring이건 JSP 개발 방식을 접목
...
- 각각의 매서드별 매핑이 존재해서 현재는 많이 사용안됨
- 전체 컨트롤러의 동일한 URL페이지명 통합일때 자주 사용
- 사용법
...
- URL http://localhost:8080/search?name=Kimage=23
- 매서드 파라미터 사용
...
- 개요
  - Spring Boot에서 HTML 화면 만들 때 활용하는 템플릿 엔진
...
```java
return "/borad/list";     // templates 밑 borad폴더 아래의 list.html 호출
```
...
```html
<html xmls:th="http://www.thymeleaf.org"></html>
```
...
- 아래의 키워드를 HTML 상에 적절하게 배치하면 됨
  ...
  - `th:if="${변수명 >= 20}"` : 제어문 IF 문. 반대로 `th:unless` 도 존재
  - `th:href="@{/students/list}"` : a 태그의 링크와 동일
  - `th:value="${객체명.필드명}"` : 수정 폼에서 사용
  - `th:object="${객체명}" + th:field=*{필드명}"` : Spring 폼 바인딩 검색
...
- Gradle 사용시 관련 파일 위치 : C:\Users\Admin\.gradle
...
- 설치 후 실행할때 회원가입이나 로그인 필요없음(Continue without an account)

![alt text](image-21.png)

- HTTP 메서드 종류 변경해서 테스트
- POST, PUT, DELETE 의 경우는 Body 클릭, raw, json 선택 후 필요데이터(json) 입력후 SEND 클릭
...
- .vscode : VS Code가 프로젝터에 필요한 설정 담는 폴더
- bin, build : 자바 컴파일후 생성되는 class 파일 위치
- gradle : gradle을 자바에서 쓸수있게 만든 라이브러리 위치
- `src/main/java` : 패키지와 자바 소스가 저장된 폴더
  - com/pknu26/httpmethod : 지정한 Group ID와 Artifact ID를 활성한 폴더(com.pknu26.httpmethod)
  - HelloController 클래스 접근 : com.hugo83.spring83.controller.HelloController 로 접근
...
- `src/main/resources` : 자바이외 HTML, CSS, JS, 이미지 파일, 환경파일 등 리소스가 되는 파일 저장하는 폴더
  ...
  - template : 스프링부트와 연계되는 tymeleaf html이나 mustache 파일 저장
  - `application.properties` : 프로젝트 환경설정, DB설정, 환경변수
- src/test/java : JUnit 테스트 도구 자바파일 저장
- .gitattributes : 깃서버에서 어떻게 처리할지
...
- Record
  ...
  - 2020년 Java 14d에서 첫 등장, 2021년 Java 16에서 정식 사용
...
1. BoardController 생성 - [소스](./day05/webboard/src/main/java/com/pknu26/webboard/entity/Board.java)
2. Borad_list.html 생성
3. Board 생성
