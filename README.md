...
- 절차지향 방식의 문제점
  ...
  - 관련된 데이터가 흩어지고,
  - 같은 형태의 변수선언이 반복되고,
  - 학생 한 면의 정보를 하나의 단위(그룹)으로 다루기 힘들고
...
- 객체(Object)
  ...
  ```java
  // Student : Integer, Int, Long과 같은 타입
  // std1 : 변수명
...
- 생성자(Contructor)
  ...
  - 없으면 자동 생성, 필요하면 클래스명과 동일한 매서드를 작성
...
- 캡슐화(Encapsulation)
  - 객체 내부의 데이터를 외부에서 마음대로 바꾸지못하게 막고, `정해진 방법으로만 접근가능`하게 만든 것
...
System.out.print("계좌 잔고는 : " + myAcc.getBalance());
...
- 상속 밭은 클래스에서 부모 클래스의 매서들 자기 방식으로 다시 정의해서 사용하는 것

```java
class Animal {
  void sound() {
    System.out.println("동물 소리"); // 부모클래스 매서드
  }
}
...
- 이노테이션 종류
  - @Override
  - @Deprecated : 이제는 사용안할 메서드 지침
  - @SupressWarning : 컴파일러에게 경고 메시지 출력하지 말것
  - @SpringBootApplication @Component, @Service, @Repository, @Controller, ... : SpringBoot 이노테이션 등

#### 메서드 오버로딩

- 같은 이름의 메서드를 매개변수 다르개 여러개 정의
...
- super : 부코 클래스 지칭
  ...
  class Dog extends Animal {
    ...
    void printName(){
      ...
      System.out.println(super.name);  // 강아지 출력
    }
  }
  ```
...
- 인터페이스 - [소스](./day02/ex04_oop/app/src/main/java/ex04_oop/iCage.java)
  ...
  - 규칙을 완전 통제, 통일할 수 있음
  - 클래스의 한 종류. 상속이라 부르지 않고 구현(implemetation)이라 칭함
...
```java
  // iCage 인터페이스를 가져다 쓰는 클래스는 반드시 아래의 매서드를 구현해야함
  interface iCage {
    void checkIn();
    void checkOut();
  }
...
- `class Cat extends Animal, Mannel` 다중 클래스 상속 불가능
...
- Code sinppet : 코딩 문법을 자동 생성해주는 기능
...
- 자동차도 만들고, 엔진도 만들고(의존성), 엔진은 만들걸 사서 조립하면(DI), 다 만들어주면(IoC)
...
- Java에서 모든 클래스의 부모 클래스(Root Class)
...
- 일반화 필요성
  - Python, JavaScript는 데이터타입 지장없음 -> 아무런 데이터나 변수에 들어감
...
class Box {
    Object value;  // 멤버 골드. 무슨 타입이든지 받을 수 있음
...
class Box<T> {        // T -> 아무타입이나 상관없음
    T value;  // 멤버 골드. 무슨 타입이든지 받을 수 있음
